...
import '../services/puzzle_engine_service.dart';
...
class _PuzzleNodeScreenState extends State<PuzzleNodeScreen>
  ...
  Future<void> _openNextPuzzle({bool allowUncleared = false}) async {
    ...
    final currentSolvedHistorically =
        currentPuzzle != null && provider.isPuzzleSolved(currentPuzzle.puzzleId);
    final nextEntry = _adjacentNavigablePuzzle(provider, forward: true);
    if (nextEntry == null ||
        (!allowUncleared && !_solved && !currentSolvedHistorically)) {
      return;
    }

    final nextState = provider.tileStateForNodeIndex(widget.node, nextEntry.key);
    await _loadPuzzle(
      nextEntry.value,
      nextEntry.key,
  ...
  Future<void> _openPreviousPuzzle() async {
    ...
    await _loadPuzzle(previousEntry.value, previousEntry.key, historicalView: true);
  }
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: isCaptureTarget
                                ? Border.all(
                                    color: targetDotColor.withValues(alpha: 0.82),
                                    width: 2,
                                  )
                                : null,
                          ),
  ...
  Widget _buildBottomActions(
    ...
    final canAdvanceToNext =
        !_busy &&
        (_solved || currentSolvedHistorically) &&
        nextPuzzle != null &&
...
class _PerspectiveEvalBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final topFlex = max(1, (playerShare * 100).round());
    final bottomFlex = max(1, (opponentShare * 100).round());
