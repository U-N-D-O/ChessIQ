...
abstract class _ChessAnalysisPageStateBase extends State<ChessAnalysisPage>
  ...
  BoardPerspective _analysisStandardPerspective = _defaultPerspective;
  ...
  static const double _menuFloorMoveSoundDelay = 0.80;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          const SizedBox(height: 10),
          Text(
            'Leaderboard privacy: the nickname and country or region you choose may be displayed publicly with your score and title. ChessIQ also sends an anonymous Firebase ID and update metadata to the backend to manage the entry. If you redeem a promo code and have an Academy nickname, ChessIQ may store that nickname privately with the anonymous Firebase ID, redeemed code, and claim timestamp so operators can track redemptions. Email, real name, and precise location are not requested, and network metadata such as IP addresses is not shown on the public leaderboard.',
            style: _creditsBodyStyle(
              visuals,
              size: 11.6,
              color: visuals.palette.text.withValues(alpha: 0.88),
  ...
  Future<void> _loadAnalysisSettingsPrefs() async {
    try {
      ...
      if (savedPerspective != null &&
        ...
        if (_perspective != BoardPerspective.headToHead) {
          _analysisStandardPerspective = _perspective;
        }
      }
      if (savedDepth != null) {
  ...
  void _endLocalFriendOnTime({required bool whiteFlagged}) {
    ...
    setState(() {
      ...
      _recordLocalFriendSeriesOutcome(outcome);
  ...
  bool get _friendClockTopShowsWhite =>
      _isLocalFriendMatchMode ? _localFriendTopSeatIsWhite : false;

  bool get _friendClockBottomShowsWhite =>
      _isLocalFriendMatchMode ? _localFriendBottomSeatIsWhite : true;

  String _localFriendSeatLabelForColor(bool white) {
    return white == _localFriendBottomSeatIsWhite ? 'BOTTOM' : 'TOP';
  }

  int _localFriendSeatScoreForColor(bool white) {
    return white == _localFriendBottomSeatIsWhite
        ? _localFriendBottomSeatWins
        : _localFriendTopSeatWins;
  }

  void _recordLocalFriendSeriesOutcome(GameOutcome outcome) {
    if (!_isLocalFriendMatchMode) {
      return;
    }
    ...
    final whiteWon = outcome == GameOutcome.whiteWin;
    final bottomSeatWon = _localFriendBottomSeatIsWhite == whiteWon;
    if (bottomSeatWon) {
      _localFriendBottomSeatWins += 1;
    } else {
      _localFriendTopSeatWins += 1;
    }
  }

  void _advanceLocalFriendSeriesToNextRound() {
    _localFriendBottomSeatIsWhite = !_localFriendBottomSeatIsWhite;
  }
  ...
  Future<void> _refreshRemoteFriendMatch({bool silent = false}) async {
    ...
    try {
      ...
      setState(() {
        _applyRemoteFriendSnapshot(invite: nextInvite, snapshot: nextSnapshot);
      });
      _startRemoteFriendSyncTimers();
  ...
  void _invalidateMoveQualityGradingPipeline({
    ...
    if (clearPendingMove) {
      ...
      _moveQualityPublishTimer?.cancel();
      _moveQualityPublishTimer = null;
      _deferredMoveQualityPublication = null;
    }
  }
  ...
  void _publishBookFastPath(_PendingMoveQualityGrading pending) {
    ...
    final deferred = _deferredMoveQualityPublication;
    if (deferred?.pending.moveIndex == pending.moveIndex &&
        deferred?.pending.uci == pending.uci) {
      _clearDeferredMoveQualityPublication();
    }
    ...
    final badgeSquare = pending.uci.length >= 4
        ...
        : null;
    final opening = _currentOpening.trim();
    var overlayMove = currentMove.copyWith(
      quality: MoveQuality.book,
      qualityExplanation: opening.isEmpty
          ? 'Opening theory'
          : 'Opening: $opening',
  ...
  void _stageMoveQualityPublication(
    _DeferredMoveQualityPublication publication,
  ) {
    _deferredMoveQualityPublication = publication;
    _moveQualityPublishTimer?.cancel();
    final remainingDelay = publication.pending.earliestPublishAt.difference(
      DateTime.now(),
    );
    if (remainingDelay <= Duration.zero) {
      _publishDeferredMoveQualityPublication();
      return;
    }
    _moveQualityPublishTimer = Timer(remainingDelay, () {
      if (!mounted) {
        return;
      }
      _publishDeferredMoveQualityPublication();
  ...
  void _clearDeferredMoveQualityPublication() {
    _moveQualityPublishTimer?.cancel();
    _moveQualityPublishTimer = null;
    _deferredMoveQualityPublication = null;
  }

  void _publishDeferredMoveQualityPublication() {
    final publication = _deferredMoveQualityPublication;
    if (publication == null) {
      _moveQualityPublishTimer?.cancel();
      _moveQualityPublishTimer = null;
      return;
    }
    if (publication.pending.earliestPublishAt.isAfter(DateTime.now())) {
      _stageMoveQualityPublication(publication);
      return;
    }
    _clearDeferredMoveQualityPublication();
    _applyMoveQualityPublication(publication);
  }

  void _applyMoveQualityPublication(
    _DeferredMoveQualityPublication publication,
  ) {
    final pending = publication.pending;
    ...
    final updatedMove = currentMove.copyWith(
      quality: quality,
      preMoveWinProbability: publication.preMoveMoverWinProbability,
      postMoveWinProbability: publication.postMoveMoverWinProbability,
      deltaWinProbabilityLoss: publication.deltaWpLoss,
    ...
    final shouldShowAssessment =
        publication.sourceRole != EngineRequestRole.backgroundConfirmation ||
  ...
  _PendingMoveQualityGrading? _capturePendingMoveQualityGrading({
    ...
    return _PendingMoveQualityGrading(
      ...
      uci: uciMove,
      moverIsWhite: moverIsWhite,
      earliestPublishAt: DateTime.now().add(_moveQualityPublishDelay),
  ...
  bool get _hasActiveSacrificeAvailabilityAlert =>
      !_isSacrificeModeActive &&
      _hasAvailableSacrificePreview &&
      _sacrificeAvailabilityAlertUntil?.isAfter(DateTime.now()) == true;

  void _updateSacrificePreviewState({
    required String fen,
    required List<EngineLine> previewLines,
  }) {
    final availabilitySignature = previewLines.isEmpty ? null : fen;
    final shouldTriggerAlert =
        !_isSacrificeModeActive &&
        availabilitySignature != null &&
        availabilitySignature != _lastSacrificeAvailabilityAlertSignature;
    setState(() {
      _sacrificePreviewFen = fen;
      _sacrificePreviewLines = previewLines;
      _lastSacrificeAvailabilityAlertSignature = availabilitySignature;
      if (availabilitySignature == null) {
        _sacrificeAvailabilityAlertUntil = null;
      } else if (shouldTriggerAlert) {
        _sacrificeAvailabilityAlertUntil = DateTime.now().add(
          _sacrificeAvailabilityAlertDuration,
        );
      }
    });
  }
  ...
  void _scheduleSacrificePreviewScan() {
    ...
    if (!_canEvaluateSacrificeAvailability) {
      if (_sacrificePreviewFen != null || _sacrificePreviewLines.isNotEmpty) {
        _updateSacrificePreviewState(
          fen: fen,
          previewLines: const <EngineLine>[],
        );
      }
      return;
    }
  ...
  Future<void> _runSacrificePreviewScan({
      ...
      for (final captureReply in captureReplies) {
        ...
        if (candidate != null) {
          ...
          if (previewLines.isNotEmpty) {
            _updateSacrificePreviewState(fen: fen, previewLines: previewLines);
    ...
    _updateSacrificePreviewState(
      fen: fen,
      previewLines: _previewLinesFromSacrificeCandidates(sacrificeCandidates),
    );
  }
  ...
  Future<void> _finalizePendingMoveQualityGrading(
    ...
    final publication = _DeferredMoveQualityPublication(
      ...
      postMoveMoverWinProbability: postMoveMoverWinProbability,
      deltaWpLoss: loss,
      sourceRole: livePostMoveRole,
    );
    if (pending.earliestPublishAt.isAfter(DateTime.now())) {
      _stageMoveQualityPublication(publication);
      return;
    }
    _clearDeferredMoveQualityPublication();
  ...
  void _showMoveQualityOverlay(MoveRecord move, int chargeDelta) {
    ...
    final overlayDuration = quality == MoveQuality.book
        ? _bookMoveQualityOverlayDuration
        : _moveQualityOverlayDuration;
    _moveQualityOverlayTimer?.cancel();
  ...
  bool get _isBlackPovActive {
    ...
    if (_isLocalFriendMatchMode && _forcesStationaryLocalFriendPerspective) {
      return !_localFriendBottomSeatIsWhite;
    }
  ...
  void _onMove(String from, String to, {String? promotion}) {
    ...
    final autoDisableSacrificeMode =
        _isSacrificeModeActive &&
        ((_sacrificePreviewFen == _genFen() &&
    ...
    setState(() {
      ...
      if (autoDisableSacrificeMode) {
        ...
        _sacrificeAvailabilityAlertUntil = null;
        _lastSacrificeAvailabilityAlertSignature = null;
      }
    });
    ...
    if (detectedOutcome != null) {
      ...
      setState(() {
        ...
        if (_isLocalFriendMatchMode) {
          _recordLocalFriendSeriesOutcome(gameOutcome);
        }
        _gameOutcome = gameOutcome;
        _gameDrawReason = detectedOutcome.drawReason;
        _botThinking = false;
  ...
  Widget _buildVsModeScreen() {
    ...
    ButtonStyle buildActionButtonStyle({
      ...
      final background = filled
          ? accent
          : Color.alphaBlend(
              accent.withValues(alpha: arcade.monochrome ? 0.14 : 0.18),
              arcade.panelAlt,
            );
    ...
    ChoiceChip buildChoiceChip({
      ...
      return ChoiceChip(
        ...
        selectedColor: Color.alphaBlend(
          accent.withValues(alpha: arcade.monochrome ? 0.12 : 0.20),
          arcade.panelAlt,
        ),
        side: BorderSide(
          color: selected
      ...
      return LayoutBuilder(
        builder: (context, constraints) {
          ...
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              buildTag(label, accent, icon: icon, filled: !arcade.monochrome),
              const SizedBox(height: 10),
              Text(
                title,
                style: buildDisplayStyle(
              ...
              const SizedBox(height: 6),
              Text(subtitle, style: buildBodyStyle(color: arcade.textMuted)),
            ],
          );
        },
      );
    ...
    Color remoteMembershipAccent(RemoteFriendMatchStatus status) {
      return switch (status) {
        ...
        RemoteFriendMatchStatus.expired ||
        RemoteFriendMatchStatus.cancelled => arcade.textMuted,
      };
    }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  detail,
                  style: buildBodyStyle(color: arcade.textMuted, size: 11.3),
                ),
              ],
            ),
          ),
    ...
    Future<void> showPlayChessInfoDialog() async {
      await showDialog<void>(
        context: context,
        builder: (dialogContext) {
          final dialogMedia = MediaQuery.of(dialogContext);
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 8),
                                Text(
                                  'Use the four main menu options to jump into the right match flow quickly.',
                                  style: buildBodyStyle(
                                    color: arcade.textMuted,
                                    size: 11.8,
                                  ),
                      ...
                      infoRow(
                        ...
                        detail:
                            'Private online match across devices with server-validated turns and synced clocks.',
                      ...
                      infoRow(
                        ...
                        detail:
                            'Pass-and-play on one device with optional timers and fixed board orientation.',
                      ...
                      infoRow(
                        ...
                        detail:
                            'Pick this option to open the quick-join panel and enter a friend invite code.',
                      ...
                      buildInsetShell(
                        accent: heroAccent,
                        child: Text(
                          'Tip: tap an option tile first, then use the setup panel below to configure and start that mode.',
                          style: buildBodyStyle(
                            color: arcade.textMuted,
                            size: 11.4,
                          ),
    ...
    Widget buildHeroPanel() {
      ...
      Widget buildModeOptionCard({
        ...
        return puzzleAcademyDecoratedInkWell(
          ...
          onTap: () {
            if (option == _VsModeQuickOption.vsCpu) {
                      ...
                      child: DecoratedBox(
                        decoration: BoxDecoration(
                          gradient: LinearGradient(
                            ...
                            colors: [
                              accent.withValues(alpha: selected ? 0.18 : 0.12),
                              Colors.transparent,
      ...
      return buildCardShell(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.sports_esports_rounded, size: 18, color: heroAccent),
                const SizedBox(width: 8),
                Text(
                  'PLAY CHESS',
                  style: buildHudStyle(
                ...
                Container(
                  ...
                  decoration: _vsBotArcadePanelDecoration(
                    ...
                    fillColor: Color.alphaBlend(
                      arcade.amber.withValues(
                        alpha: useMonochrome ? 0.08 : 0.16,
                      ),
                      arcade.panelAlt,
                    ),
                  ),
                  child: Text(
                    useMonochrome ? 'MONO' : 'ARCADE',
                    style: buildHudStyle(
                      color: arcade.amber,
                      size: 9.6,
                      weight: FontWeight.w800,
    ...
    Widget buildFocusedModePanel() {
      Widget buildCompactBotPanel() {
        ...
        return buildCardShell(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              buildSectionTitle('CPU MATCH SETUP', accent),
              const SizedBox(height: 10),
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: [
                  buildTag(
                    'BOT ${selectedBot?.name.toUpperCase() ?? 'NOT SELECTED'}',
                    accent,
                    icon: Icons.smart_toy_outlined,
                  ),
                  buildTag(
      ...
      Widget buildCompactRemotePanel() {
        ...
        return buildCardShell(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              buildSectionTitle('ONLINE MATCH SETTINGS', remoteAccent),
              const SizedBox(height: 10),
              Wrap(
                spacing: 8,
                runSpacing: 8,
                    ...
                    for (
                      ...
                      buildChoiceChip(
                        ...
                        onSelected: mainActionBusy
                            ...
                            : (_) {
                                unawaited(
                                  _setLocalFriendTimeControlIndex(index),
                                );
                              },
                        accent: remoteAccent,
                      ),
      ...
      return switch (_selectedVsModeQuickOption) {
        _VsModeQuickOption.crossDevice => buildCompactRemotePanel(),
        _VsModeQuickOption.sharedScreen => buildCompactLocalPanel(),
        _VsModeQuickOption.vsCpu => buildCompactBotPanel(),
        _VsModeQuickOption.quickJoin => buildQuickJoinPanel(),
      };
    }
    ...
    Widget buildRemoteModeCard() {
      Widget buildRecentMatchesContent() {
        ...
        final listHeight = min(336.0, 96.0 * _remoteFriendMemberships.length);
        ...
        return SizedBox(
          ...
          child: Scrollbar(
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                return buildRemoteMembershipTile(
                  _remoteFriendMemberships[index],
                );
              },
            ),
        ...
        child: Wrap(
          ...
          children: [
            ...
            buildSnapshotTile(
              ...
              detail:
                  'Your color request is applied before piece selection opens.',
            ...
            buildSnapshotTile(
              ...
              detail:
                  'Finished or expired matches are swept away automatically.',
              icon: Icons.auto_delete_rounded,
              accent: arcade.amber,
              minWidth: 170,
            ),
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                if (constraints.maxWidth < 470) {
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [modeTags, const SizedBox(height: 10), infoPod],
                  );
                }
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            Color.alphaBlend(
              arcade.cyan.withValues(alpha: useMonochrome ? 0.02 : 0.06),
              scheme.surface,
            ),
            scheme.surface,
            Color.alphaBlend(
              arcade.amber.withValues(alpha: useMonochrome ? 0.02 : 0.05),
              scheme.surfaceContainerHighest,
            ),
          ],
        ...
        children: [
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color: arcade.cyan.withValues(alpha: isDark ? 0.10 : 0.06),
              ),
            ),
          ),
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color: arcade.amber.withValues(alpha: isDark ? 0.08 : 0.05),
              ),
            ),
          ),
          SafeArea(
  ...
  Widget _buildFriendClockPanel({
    ...
    final facesOppositePlayer =
        _isLocalFriendMatchMode && white == _friendClockTopShowsWhite;
  ...
  int _clockLowTimeWarningThresholdMs(Duration initialTime) {
    final warningSeconds = max(
      12,
      min(90, (initialTime.inSeconds / 6).round()),
    );
    return warningSeconds * Duration.millisecondsPerSecond;
  }

  int _clockCriticalTimeWarningThresholdMs(Duration initialTime) {
    final criticalSeconds = max(
      5,
      min(30, (initialTime.inSeconds / 12).round()),
    );
    return criticalSeconds * Duration.millisecondsPerSecond;
  }

  double _clockLowTimePulseStrength({
    required Duration remaining,
    required Duration? initialTime,
  }) {
    if (initialTime == null || initialTime <= Duration.zero) {
      return 0;
    }

    final remainingMs = max(0, remaining.inMilliseconds);
    final warningThresholdMs = _clockLowTimeWarningThresholdMs(initialTime);
    if (remainingMs >= warningThresholdMs) {
      return 0;
    }

    final pulse = (sin(_pulseController.value * pi * 2) + 1) / 2;
    final warningProgress =
        1 - (remainingMs / warningThresholdMs).clamp(0.0, 1.0);
    var strength =
        Curves.easeOut.transform(warningProgress.clamp(0.0, 1.0)) *
        (0.12 + pulse * 0.12);

    final criticalThresholdMs = _clockCriticalTimeWarningThresholdMs(
      initialTime,
    );
    if (remainingMs <= criticalThresholdMs) {
      final criticalProgress =
          1 - (remainingMs / criticalThresholdMs).clamp(0.0, 1.0);
      strength +=
          Curves.easeOut.transform(criticalProgress.clamp(0.0, 1.0)) *
          (0.08 + pulse * 0.10);
    }
  ...
  Widget _buildLocalFriendClockCard({required bool white}) {
    ...
    final accent = isWinner
        ...
        : scheme.onSurface.withValues(alpha: 0.72);
    final label =
        '${_localFriendSeatLabelForColor(white)} | ${white ? 'WHITE' : 'BLACK'}';
    final seatScore = _localFriendSeatScoreForColor(white);
    ...
    return AnimatedBuilder(
      animation: _pulseController,
      builder: (context, child) {
        final remaining = _hasTimedLocalFriendClock
            ? _localFriendDisplayedClock(white: white)
            : Duration.zero;
        final warningStrength = isActive && _gameOutcome == null
            ? _clockLowTimePulseStrength(
                remaining: remaining,
                initialTime: _selectedLocalFriendTimeControl.initialTime,
              )
            : 0.0;
        const warningColor = Color(0xFFE66565);
        final displayAccent =
            Color.lerp(accent, warningColor, warningStrength) ?? accent;
        final fillColor = Color.alphaBlend(
          Color.lerp(
                accent.withValues(alpha: isDark ? 0.16 : 0.10),
                warningColor.withValues(alpha: isDark ? 0.22 : 0.14),
                warningStrength,
              ) ??
              accent.withValues(alpha: isDark ? 0.16 : 0.10),
          scheme.surface,
        ...
        final borderColor =
            Color.lerp(
              ...
              warningColor.withValues(alpha: isDark ? 0.70 : 0.58),
              warningStrength,
            ) ??
            accent.withValues(alpha: 0.46);
        final shadowColor =
            Color.lerp(
              accent.withValues(alpha: isActive ? 0.20 : 0.10),
              warningColor.withValues(alpha: isActive ? 0.24 : 0.12),
              warningStrength,
            ) ??
            accent.withValues(alpha: isActive ? 0.20 : 0.10);
        final timeTextColor =
            Color.lerp(
              scheme.onSurface,
              warningColor.withValues(alpha: 0.96),
              warningStrength * 0.24,
            ) ??
            scheme.onSurface;
        final detailColor =
            Color.lerp(
              scheme.onSurface.withValues(alpha: 0.66),
              warningColor.withValues(alpha: 0.88),
              warningStrength * 0.16,
            ) ??
            scheme.onSurface.withValues(alpha: 0.66);
        final timeText = _hasTimedLocalFriendClock
            ? _formatLocalFriendClock(remaining)
            : 'UNTIMED';
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: shadowColor,
                blurRadius: isActive ? 18 + warningStrength * 5 : 12,
                offset: const Offset(0, 6),
              ),
            ],
          ),
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(999),
                        border: Border.all(
                          color: displayAccent.withValues(alpha: 0.34),
                        ),
                      ),
                      child: Text(
                        'S $seatScore',
          ...
          children: [
            CustomPaint(
              ...
              painter: EnergyArrowPainter(
                ...
                pixelKnightTurnedHeadScale: previewLines.isEmpty ? 1.35 : 1.0,
                boardInset: boardInset,
              ),
            ),
            ..._botGhostArrows.map(
                    ...
                    child: AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        final hasAvailableSacrifice =
                            _hasAvailableSacrificePreview;
                        final subtleSacrificeAlertActive =
                            _hasActiveSacrificeAvailabilityAlert;
                        final flashProgress =
                        ...
                        final alertRed = Color.lerp(
                          idleIconColor,
                          const Color(0xFFD97A7A),
                          0.16 + (pulseProgress * 0.10),
                        )!;
  ...
  Widget _buildSuggestionTriggerButton() {
    ...
    if (_isHeadToHeadPerspective) {
      return _buildHeadToHeadSuggestionLockButton();
    }
  ...
  Future<void> _openSettings({
    ...
    if (isBoardAnalysisPage && _perspective != BoardPerspective.headToHead) {
      _analysisStandardPerspective = _perspective;
    }
      ...
      boardThemeSelectorBuilder: (setSheetState, markChanged) {
        return Wrap(
          ...
          children: _availableBoardThemes
              .map(
                (mode) => _boardThemeOption(mode, setSheetState, markChanged),
              )
              .toList(),
        );
      },
  ...
  Future<void> _performResetWithSponsoredBreak() async {
    ...
    final advanceCompletedLocalFriendRound =
        wasLocalFriendMatch && _gameOutcome != null;
    if (shouldAttemptAd) {
    ...
    if (!mounted) return;
    setState(() {
      if (advanceCompletedLocalFriendRound) {
        _advanceLocalFriendSeriesToNextRound();
      }
      _resetBoard();
  ...
  String _promoUnlockLabel(String unlockKey) {
    switch (unlockKey) {
      case 'themePackOwned':
        return 'Theme Pack';
      case 'sakuraBoardOwned':
        return 'Sakura board';
      case 'tropicalBoardOwned':
        return 'Tropical board';
      case 'piecePackOwned':
        return 'Piece Set Pack';
      case 'tuttiFruttiOwned':
        return 'Tutti Frutti pieces';
      case 'spectralOwned':
        return 'Spectral pieces';
      case 'monochromePiecesOwned':
        return 'Monochrome pieces';
      case 'pixelArrowThemeOwned':
        return '8-Bit arrows';
      case 'heavyArrowThemeOwned':
        return 'Massive 3D arrows';
      case 'sacrificeModeOwned':
        return 'Sacrifice Mode';
      case 'adFreeOwned':
        return _cleanPlayPassTitle;
      case 'academyTuitionPassOwned':
  ...
  bool _applyPromoUnlockReward(String unlockKey) {
    ...
    void applyChange(VoidCallback mutation) {
      if (mounted) {
        setState(() {
          mutation();
          _normalizeUnlockedThemes();
    ...
    switch (unlockKey) {
      case 'themePackOwned':
        if (_themePackOwned) return false;
        applyChange(() {
          _themePackOwned = true;
        });
        break;
      case 'sakuraBoardOwned':
        if (_sakuraBoardOwned) return false;
        applyChange(() {
          _sakuraBoardOwned = true;
        });
        break;
      case 'tropicalBoardOwned':
        if (_tropicalBoardOwned) return false;
        applyChange(() {
          _tropicalBoardOwned = true;
        });
        break;
      case 'piecePackOwned':
        if (_piecePackOwned) return false;
        applyChange(() {
          _piecePackOwned = true;
        });
        break;
      case 'tuttiFruttiOwned':
        if (_tuttiFruttiOwned) return false;
        applyChange(() {
          _tuttiFruttiOwned = true;
        });
        break;
      case 'spectralOwned':
        if (_spectralOwned) return false;
        applyChange(() {
          _spectralOwned = true;
        });
        break;
      case 'monochromePiecesOwned':
        if (_monochromePiecesOwned) return false;
        applyChange(() {
          _monochromePiecesOwned = true;
        });
        break;
      case 'pixelArrowThemeOwned':
        if (_pixelArrowThemeOwned) return false;
        applyChange(() {
          _pixelArrowThemeOwned = true;
        });
        break;
      case 'heavyArrowThemeOwned':
        if (_heavyArrowThemeOwned) return false;
        applyChange(() {
          _heavyArrowThemeOwned = true;
        });
        break;
      case 'sacrificeModeOwned':
        if (_sacrificeModeOwned) return false;
        applyChange(() {
          _sacrificeModeOwned = true;
        });
        break;
      case 'adFreeOwned':
        if (_adFreeOwned) return false;
        applyChange(() {
          _adFreeOwned = true;
        });
        break;
      case 'academyTuitionPassOwned':
        if (_academyTuitionPassOwned) return false;
        applyChange(() {
          _academyTuitionPassOwned = true;
        });
        break;
    ...
    if (changed && unlockKey == 'sacrificeModeOwned') {
      _refreshAnalysisForCurrentPosition();
    }
    return changed;
  }
  ...
  Future<String> _applyPromoCodeReward(PromoCodeRedemptionResult result) async {
    ...
    final unlockKey = result.unlockKey;
    if (unlockKey != null && unlockKey.isNotEmpty) {
      final changed = _applyPromoUnlockReward(unlockKey);
      unlockChanged = unlockChanged || changed;
      fragments.add(
        changed
            ? '${_promoUnlockLabel(unlockKey)} unlocked'
            : '${_promoUnlockLabel(unlockKey)} already owned',
      );
    }
    ...
    return 'Promo redeemed: ${fragments.join(' and ')}.';
  }
  ...
  Future<void> _openStore({
    ...
    String? promoRedeemMessage;
    bool promoRedeemMessageIsError = false;
    bool promoRedeemInProgress = false;
    try {
      await showModalBottomSheet<void>(
        ...
        builder: (_) => StatefulBuilder(
          builder: (ctx, setL) {
            rewardCooldownTicker ??= Timer.periodic(
            ...
            Future<void> redeemPromoCode() async {
              ...
              if (code.isEmpty) {
                setL(() {
                  promoRedeemMessage = 'Enter a promo code before redeeming.';
                  promoRedeemMessageIsError = true;
                });
                return;
              }

              setL(() {
                promoRedeemInProgress = true;
                promoRedeemMessage = null;
                promoRedeemMessageIsError = false;
              });

              try {
                final result = await PromoCodeService.instance.redeemCode(code);
                final rewardSummary = await _applyPromoCodeReward(result);
                if (!ctx.mounted) {
                  return;
                }
                promoCodeController.clear();
                setL(() {
                  promoRedeemInProgress = false;
                  promoRedeemMessage = rewardSummary;
                  promoRedeemMessageIsError = false;
                });
                _addLog('Promo code ${result.code} redeemed');
              } on PromoCodeException catch (error) {
                if (!ctx.mounted) {
                  return;
                }
                setL(() {
                  promoRedeemInProgress = false;
                  promoRedeemMessage = error.message;
                  promoRedeemMessageIsError = true;
                });
                _addLog('Promo code redeem failed: ${error.message}');
              } catch (error) {
                if (!ctx.mounted) {
                  return;
                }
                setL(() {
                  promoRedeemInProgress = false;
                  promoRedeemMessage = 'Could not redeem promo code right now.';
                  promoRedeemMessageIsError = true;
                });
                _addLog('Promo code redeem failed: $error');
              }
            }
            ...
            const storeBottomActionBarReserve = 148.0;
                    ...
                    children: [
                      SingleChildScrollView(
                        padding: EdgeInsets.fromLTRB(
                          ...
                          12,
                          20,
                          storeBottomActionBarReserve + bottomSafePadding,
                        ),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.stretch,
                          children: [
                              ...
                              child: Column(
                                ...
                                children: [
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            const SizedBox(height: 2),
                                            Text(
                                              'Redeem coins or a special store unlock',
                                              style: TextStyle(
                                                color: scheme.onSurface
                                  ...
                                  TextField(
                                    ...
                                    enabled: !promoRedeemInProgress,
                                    textCapitalization:
                                        TextCapitalization.characters,
                                    textInputAction: TextInputAction.done,
                                    maxLength: 32,
                                    onChanged: (_) {
                                      if (promoRedeemMessage != null) {
                                        setL(() {
                                          promoRedeemMessage = null;
                                        });
                                      }
                                  ...
                                  SizedBox(
                                    width: double.infinity,
                                    child: FilledButton(
                                      onPressed: promoRedeemInProgress
                                          ? null
                                          : () async {
                                              await redeemPromoCode();
                                            },
                                  ...
                                  Text(
                                    promoRedeemMessage ?? ' ',
                                    style: TextStyle(
                                      color: promoRedeemMessage == null
                                          ? Colors.transparent
                                          : promoRedeemMessageIsError
                      ...
                      Align(
                        ...
                        child: Padding(
                          ...
                          child: _buildStoreBottomActionBar(
                            context: ctx,
                            backgroundSurface: sheetSurface,
  ...
  Widget _buildStoreBottomActionBar({
    required BuildContext context,
    required Color backgroundSurface,
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 2),
              Text(
                'Purchases and theme selections apply instantly. Tap Close Store when you are done.',
  ...
  Widget _buildHeadToHeadSuggestionLockButton() {
    final theme = Theme.of(context);
    final scheme = theme.colorScheme;
    final isDark = theme.brightness == Brightness.dark;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        color: background,
        border: Border.all(
          color: scheme.outline.withValues(alpha: isDark ? 0.34 : 0.26),
          width: 1.6,
        ),
        boxShadow: [
          BoxShadow(
    ...
    return Row(
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: accent.withValues(alpha: compact ? 0.12 : 0.14),
            borderRadius: BorderRadius.circular(999),
  ...
  void _applyPerspectiveSelection(BoardPerspective perspective) {
    if (perspective != BoardPerspective.headToHead) {
      _analysisStandardPerspective = perspective;
    } else if (_perspective != BoardPerspective.headToHead) {
      _analysisStandardPerspective = _perspective;
    }
  ...
  Widget _perspectiveOption(
    ...
    final selectedPerspective = _perspective == BoardPerspective.headToHead
        ? (_analysisStandardPerspective == BoardPerspective.headToHead
              ? _defaultPerspective
              : _analysisStandardPerspective)
        : _perspective;
    final sel = selectedPerspective == p;
    final scheme = Theme.of(context).colorScheme;
    Widget kingWidget;
    ...
    } else if (p == BoardPerspective.headToHead) {
      kingWidget = _buildHeadToHeadGlyph(context, iconSize: 13, labelSize: 8);
    } else {
      kingWidget = Row(
        mainAxisSize: MainAxisSize.min,
        children: [
  ...
  Widget _pieceImage(
    ...
    Widget maybeOrientForHeadToHeadPerspective(Widget child) {
      ...
      if (!_isLockedHeadToHeadPerspective) {
        return child;
      }

      final shouldRotateForTopSeat = _isLocalFriendMatchMode
          ? isBlackPiece == _localFriendBottomSeatIsWhite
          : isBlackPiece;

      if (!shouldRotateForTopSeat) {
        return child;
      }
